...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-perfectcoin-host/README.md).
...
in the perfectcoin repository under 'Fetch and build inputs' to install sources which require
...
To build PerfectCoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the perfectcoin repository.
...
For example:
```bash
URL=https://github.com/laanwj/perfectcoin.git
...
and the perfectcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../perfectcoin/contrib/gitian-descriptors/*|sort|uniq )
...
PTCPATH=/some/root/path/perfectcoin.git
SIGPATH=/some/root/path/perfectcoin-detached-sigs.git

./bin/gbuild --url perfectcoin=${PTCPATH},signature=${SIGPATH} ../perfectcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
